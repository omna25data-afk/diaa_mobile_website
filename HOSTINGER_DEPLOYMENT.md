...
| أمر التثبيت | `pnpm install --ignore-scripts` |
...
### بديل phpMyAdmin عند غياب Node.js في SSH

إذا كانت طرفية SSH لا تعرض `node` أو `pnpm`، افتح phpMyAdmin من قسم MySQL في Hostinger، واختر قاعدة البيانات الجديدة، ثم افتح تبويب **Import** واختر الملف `database/phpmyadmin_import.sql` من المستودع. ينشئ هذا الملف جميع الجداول ويضيف المحتوى الافتتاحي بصورة آمنة قابلة لإعادة الاستيراد دون تكرار السجلات ذات المفاتيح الفريدة.
...
> تستخدم هذه النسخة **حزمة إنتاج مسبقة البناء** مدرجة في المستودع تحت `dist/`. لا تشغّل Hostinger Vite أو esbuild وقت النشر؛ بل يتحقق `pnpm run build` فقط من وجود الحزمة الجاهزة. أبقِ أمر التثبيت `pnpm install --ignore-scripts` لتفادي تنفيذ ملفات ثنائية أصلية غير مطلوبة لتشغيل الإنتاج.

عند تطوير الموقع لاحقًا، نفّذ محليًا `pnpm run build:local` ثم ارفع ملفات `dist/` المحدثة مع تغييرات المصدر قبل إعادة النشر.
