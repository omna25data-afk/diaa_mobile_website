...
> تستخدم هذه النسخة **حزمة إنتاج مسبقة البناء** مدرجة في المستودع تحت `dist/`. لا تشغّل Hostinger Vite أو esbuild وقت النشر؛ بل يتحقق `npm run build` فقط من وجود الحزمة الجاهزة. أبقِ أمر التثبيت `pnpm install --ignore-scripts` لتفادي تنفيذ ملفات ثنائية أصلية غير مطلوبة لتشغيل الإنتاج.

عند تطوير الموقع لاحقًا، نفّذ محليًا `npm run build:local` ثم ارفع ملفات `dist/` المحدثة مع تغييرات المصدر قبل إعادة النشر.
